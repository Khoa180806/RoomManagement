...
export function DateSelect({ value, onChange, label }: DateSelectProps) {
  const [parts, updateDraft] = useDraftParts(value, parseDate);
  const years = getYears();
  const days = getDays();

  function selectPart(next: { year: string; month: string; day: string }) {
    updateDraft(next);
  ...
  return (
    <div className="flex gap-2">
      <select className={selectClassName} value={parts.day} onChange={(event) => selectPart({ year: parts.year, month: parts.month, day: event.target.value })} aria-label={`${label} - Ngày`}>
        <option value="">Ngày</option>
        {days.map((day) => <option key={day} value={day.padStart(2, "0")}>{day}</option>)}
      </select>
      <select className={selectClassName} value={parts.month} onChange={(event) => selectPart({ year: parts.year, month: event.target.value, day: parts.day })} aria-label={`${label} - Tháng`}>
        <option value="">Tháng</option>
        {VIETNAMESE_MONTHS.map((monthName, index) => <option key={monthName} value={String(index + 1).padStart(2, "0")}>{monthName}</option>)}
      </select>
      <select className={selectClassName} value={parts.year} onChange={(event) => selectPart({ year: event.target.value, month: parts.month, day: parts.day })} aria-label={`${label} - Năm`}>
