...
type PaymentFormProps = {
  ...
  paymentOnTime: boolean | undefined;
...
export function PaymentForm({
  ...
  // Spec cho phép xác nhận thanh toán cho hóa đơn PENDING hoặc OVERDUE.
  const payableBills = bills.filter((bill) => bill.status === "PENDING" || bill.status === "OVERDUE");
  ...
  return (
    <section className="border border-line bg-paper p-6 shadow-[0_8px_20px_rgba(48,41,30,0.04)] md:p-8" aria-labelledby="payment-title">
      ...
      {payableBills.length === 0 ? <EmptyState message="Không có hóa đơn nào chờ thanh toán." /> : <form onSubmit={onSubmit} noValidate>
        <fieldset className="mb-6 border-0 p-0"><legend className="mb-3 text-sm font-bold text-ink">Chọn hóa đơn</legend><label className="grid gap-2 text-sm font-semibold text-ink" htmlFor="bill-select"><span>Hóa đơn <b className="text-clay" aria-hidden="true">*</b></span><select id="bill-select" className="min-h-12 w-full rounded-[0.35rem] border border-line-strong bg-white px-3 py-2 text-ink focus:border-clay focus:outline-none focus:ring-4 focus:ring-focus" value={selectedBillId ?? ""} onChange={(event) => onBillSelect(event.target.value || null)} required><option value="">-- Chọn hóa đơn --</option>{payableBills.map((bill) => <option key={bill.id} value={bill.id}>Kỳ {bill.period}{bill.status === "OVERDUE" ? " (quá hạn)" : ""} - {formatMoney(bill.totalAmount)}</option>)}</select></label></fieldset>
        <fieldset className="mb-6 border-0 p-0"><legend className="mb-3 text-sm font-bold text-ink">Thông tin thanh toán</legend><div className="grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-semibold text-ink"><span id="paid-at-label">Ngày thanh toán <b className="text-clay" aria-hidden="true">*</b></span><DateSelect label="Ngày thanh toán" value={paidAt} onChange={onPaidAtChange} /></label><label className="grid gap-2 text-sm font-semibold text-ink" htmlFor="payment-note"><span>Ghi chú</span><input id="payment-note" className="min-h-12 rounded-[0.35rem] border border-line-strong bg-white px-3 py-2 text-ink focus:border-clay focus:outline-none focus:ring-4 focus:ring-focus" type="text" value={note} onChange={(event) => onNoteChange(event.target.value)} placeholder="Không bắt buộc" /></label></div></fieldset>
      ...
      {(payments.length > 0 || paymentOnTime !== undefined || paymentTotalPages > 1) && <PaymentHistory payments={payments} currentPage={paymentPage} totalPages={paymentTotalPages} onTime={paymentOnTime} onPageChange={onPaymentPageChange} onTimeChange={onPaymentFilterChange} isLoading={isLoadingPayments} />}
    </section>
  );
}
